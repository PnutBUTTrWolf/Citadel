...
export class GtClient {
	...
	async getWorkflows(): Promise<import('./cli/contracts').GtMolecule[]> {
		try {
			const output = await this.exec(['workflow', 'list', '--json']);
			const data = JSON.parse(output);
			return Array.isArray(data) ? data : [];
		} catch {
			return [];
	...
	async getFormulas(): Promise<import('./cli/contracts').GtFormula[]> {
		try {
			const output = await this.exec(['workflow', 'formulas', '--json']);
			const data = JSON.parse(output);
			return Array.isArray(data) ? data : [];
		} catch {
			return [];
	...
	async cookMolecule(formulaId: string, params?: Record<string, string>): Promise<string> {
		const args = ['workflow', 'cook', formulaId];
		if (params) {
	...
	async pourMolecule(moleculeId: string, rig: string): Promise<string> {
		return this.exec(['workflow', 'pour', moleculeId, '--rig', rig]);
	}

	// ---- Activity Feed --------------------------------------------------------

	async getActivityFeed(): Promise<import('./cli/contracts').GtActivityEvent[]> {
		try {
			const output = await this.exec(['activity', '--json', '--limit', '50']);
			const data = JSON.parse(output);
			return Array.isArray(data) ? data : [];
		} catch {
			return [];
